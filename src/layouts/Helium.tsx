...
interface HeliumProps {
    side: boolean,
    onSide: (side: boolean) => void
}

export const Helium: FC<HeliumProps> = ({ side, onSide, children }) => (
...
export const Header: HeaderComponent<{}> = ({ children }) => (
    <header className="h-16 flex-none flex flex-row items-stretch">
        {children}
    </header>
);
...
export const Main: MainComponent<{}> = ({ children }) => {
    const context = useContext(Context);
    return (
        <main className={classnames(!context.side ? 'pl-0 md:pl-56' : 'pl-0', 'flex-grow flex flex-row overflow-y-scroll text-content bg-content-bg')}>
            {children}
        </main>
    );
}
