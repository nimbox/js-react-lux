...
Use a the viewport hook when requiring the size of the current window.  For this
to work, your root element needs to be wrapped inside `ViewportProvider` that
provides the context with the listener for the resize event.
...
<Preview>
    <Story name="Basic" parameters={{ decorators: [(Story) => <ViewportProvider>{<Story/>}</ViewportProvider>] }}>
        {() => {
...
<Preview>
    <Story name="With Wait" parameters={{ decorators: [(Story) => <ViewportProvider wait={2000}>{<Story/>}</ViewportProvider>] }}>
        {() => {
            const size = useViewport();
            return (<div>{JSON.stringify(size)}</div>);
        }}
