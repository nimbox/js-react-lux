...
<Meta 
    title="Component/controls/FieldPopper" 
...
<Canvas>
    <Story name="Variant Inlined Without Full Width">
        <div>
            Please select <FieldPopper variant="inlined" end={<AngleDownIcon className="stroke-1"/>} defaultShow={true} withArrow={true} renderPopper={() => <div className="p-4">World!</div>} className="inline-block">user</FieldPopper> responsible for the task.
        </div>
    </Story>
</Canvas>
