        ...
        if (e.target) {
            ...
            if (items) {
                ...
                new Promise((resolve, reject) => {
                    ...
                    .then(results => { setSearchResults(results as T[]); setInternalLoading(false) })
                    .catch(error => { setInternalError(error); });
            } else {
                if (searchItems)
                    Promise.resolve(searchItems(e.target.value))
                        .then(results => setSearchResults(results))
                        .catch(error => setInternalError(internalError ? internalError + ', ' + error : error));
            }
        }
    }, [items, value]);
                        ...
                        {recentValues &&
                            <ul className="pb-2">
                                {recentValues.map((value, i) => (
                                    <li className={classnames('cursor-pointer', cursor === i && 'bg-primary-400')} onClick={(e) => handleClick(e, inputRef, value)}>{doRenderItem(value)}</li>
                                ))}
                            </ul>}

                        {internalLoading && <Loading colorClassName="text-primary-500" scale="base" />}

                        {internalError && <div>{internalError}</div>}

                        {!internalLoading && !internalError && searchResults &&
                            <ul className="pt-2">
