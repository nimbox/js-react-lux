...
export const filteredColors = (value) => {
    if (value === '') {
        ...
        const matcher = createQueryMatcher(value);
        return [ 
...
Controlled and synchronous component used by the more advanced `Choose`
component. The objective behind this component is to provide a very customizable
(albeit simple) interface for option selection with search and potential
creation.
...
This component only displays the groups, options, and selected option. 
...
It is also simple to pass two sets of options.

<Canvas>
    <Story name="Choose Option List Group">
        <ChooseOptionList
            onChoose={option => action('onChoose')(option)}
...
<Canvas>
    <Story name="Choose Option List Selected">
        <ChooseOptionList 
            onChoose={option => action('onChoose')(option)}
            options={[['Yellow', 'Blue', 'Red'], ['Green', 'Purple', 'Orange']]}
            selected={[0, 0]}
...
<Canvas>
    <Story name="Choose Option List Selected AutoScroll">
        <ChooseOptionList 
            onChoose={option => action('onChoose')(option)}
            options={[['Yellow', 'Blue', 'Red'], ['Green', 'Purple', 'Orange']]}
            selected={[1, 2]}
...
This component uses the `ChooseOptionList` and adds a controlled search input
and footer. The footer is mainly used to create options not present in the list.
This is still a synchronous component but it provides the ui to signal `loading`
and `error` states potentially used when doing asynchronous operations.

Going from a simple choose option list to a choose option brings a search
capability and a footer that is usually used for creating elements that where
not found on the list. Going from one to the other only requires setting up the
state of the search bar and the search function.

The `ChooseOption`, when being used with search, requires that the options are
filtered with a given query string.
...
<Canvas>
    <Story name="Choose Option With Search">
        {() => {
            ...
            const options = useCallback((value) => [ value ? 
