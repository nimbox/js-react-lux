...
export const DatePicker: FC<Props> = ({ name, value, onChange, shortcuts, placeholder }) => {
    ...
    const months = ready ? t('months', { returnObjects: true }) as string[] : null;
    const days = ready ? t('shortDays', { returnObjects: true }) as string[] : null;
                    ...
                    <div className="flex flex-row">
                        ...
                        {shortcuts &&
                            <div className="flex flex-col justify-between bg-gray-300 cursor-pointer">
                                {namedDays.map((s, i) => <div key={i} onClick={() => handleClickDate(s.date(new Date(today)))} className="px-2 hover:text-white hover:bg-secondary-500">{t(`namedDays.${s.label}`)}</div>)}
                            </div>
                        }
